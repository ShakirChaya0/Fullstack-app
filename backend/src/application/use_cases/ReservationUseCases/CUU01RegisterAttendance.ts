...
export class CUU01RegisterAttendance {
    ...
    async execute(name: string, lastname: string): Promise<Reservation[]> {
        ...
        if (!reservatioClient) throw new NotFoundError('El cliente no tiene ninguna reserva reliza para el dia de hoy');
        ...
        if (reservationsToday.length === 0) throw new NotFoundError('No se ha encontrado ninguna reserva para ese cliente.'); 
