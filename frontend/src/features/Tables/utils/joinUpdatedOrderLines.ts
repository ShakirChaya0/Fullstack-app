...
export const consolidateOrderLines = (orderLines: LineaPedido[]) => {
    ...
    orderLines.forEach((line) => {
        ...
        if (consolidatedMap.has(key)) {
            ...
            // Acumular lineNumbers si existen
            if (line.lineNumbers) {
                if (!existingLine.lineNumbers) {
                    existingLine.lineNumbers = [];
                }
                existingLine.lineNumbers.push(...line.lineNumbers);
            }
        } else {
...
export const rebuildOrderWithConsolidatedLines = (
    ...
    const newOrderLines = consolidatedOrderLines
        .map((consolidatedLine) => {
            ...
            if (matchingPrevLine) {
                return {
                    ...
                    // ✅ Preservar lineNumbers: usar los consolidados si existen, sino del anterior
                    lineNumbers:
                        consolidatedLine.lineNumbers ??
                        matchingPrevLine.lineNumbers,
                };
            }
