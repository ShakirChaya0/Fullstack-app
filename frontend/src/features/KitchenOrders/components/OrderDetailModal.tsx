...
export default function OrderDetailModal({
    ...
    const groupedLines = order.lineasPedido.reduce<
        Record<string, typeof order.lineasPedido>
    >((acc, line) => {
                ...
                <div className="bg-[#F5F5F5] w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col outline-none">
                    <header className="flex items-center justify-between p-4 border-b border-[#E0E0E0] bg-[#FFFFFF] rounded-t-2xl">
                        <h2
                            ...
                            className="text-2xl font-bold text-[#1D1F21]"
                        >
                            Pedido #{order.idPedido}
                        </h2>
                        <button
                            onClick={onClose}
                            className="cursor-pointer text-[#B0B0B0] hover:text-[#FF6600] transition-colors text-3xl font-light"
                        >
                            &times;
                        </button>
                    </header>
                            ...
                            (type) =>
                                groupedLines[type] && (
                                    <div key={type} className="mb-4">
                                        <h3 className="text-lg font-semibold text-[#115E59] mb-2 border-b border-[#CCCCCC] pb-1 capitalize">
                                            {type.replace("_", " ")}
                                        </h3>
                                        {groupedLines[type].map((line) => (
                                            <OrderLineItem
                                                key={`${line.nroLinea}_${line.estado}_${line.cantidad}`}
                                                line={line}
