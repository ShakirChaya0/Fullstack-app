...
export default function OrderCard({ order, onSelect }: OrderCardProps) {
    ...
    const completedLines = order.lineasPedido.filter(
        (l) => l.estado === "Terminada",
    ).length;
    ...
    return (
        ...
        >
            <header className="flex justify-between items-start mb-4">
                <h3 className="font-bold text-xl text-[#1D1F21]">
                    Pedido #{order.idPedido}
                </h3>
                <span
                    className={`px-3 py-1 text-lg font-mono rounded-full ${timerColorClass}`}
                >
                    {elapsedTime}
                </span>
            </header>
            ...
            <div className="flex-grow space-y-2 mb-4 border-t border-b border-[#E0E0E0] py-3 max-h-[150px] overflow-y-auto">
                {order.lineasPedido.map((line, index) => (
                    <div
                        key={index}
                        className="flex items-center text-[#444648]"
                    >
                        <span
                            className={`font-semibold mr-2 text-sm ${statusColors[line.estado].text}`}
                        >
                            {line.cantidad}x
                        </span>
                        <span
                            className={`text-sm ${statusColors[line.estado].text} ${line.estado === "Terminada" ? "line-through" : ""}`}
                        >
                            {line.nombreProducto}
                        </span>
                    </div>
            ...
            {order.observaciones && (
                <p className="text-sm text-[#FF6600] bg-[#FFEFE5] p-2 rounded-md mb-4">
                    ⚠️ Con observaciones
                </p>
            )}
