...
import { Button, CircularProgress, type SelectChangeEvent } from "@mui/material";
...
export default function SuggestionsPage() {
    ...
    const suggestions = data ? [...data].sort((a, b) => {
        switch(sortBy) {
            case "latest":
    ...
    const handleFilterChange = (newFilter: SuggFilters) => {
        setFilter(newFilter);
    }
        ...
        <>
            <main className="flex flex-col items-center justify-start w-full p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-10 w-full max-w-7xl my-4">
                    <Button variant="contained" startIcon={<ControlPointIcon/>}>Crear Sugerencia</Button>
                    <div className="flex flex-col sm:flex-row gap-4">
                        <SuggestionsFilters filter={filter} onFilterChange={handleFilterChange}/>
                        <SortBySelect sortBy={sortBy} onSortChange={handleSort}/>
                    </div>
                </div>
