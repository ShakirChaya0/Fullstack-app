...
const minDate = getLocalISODate(tomorrow);
...
export default function ActionForm({
    ...
    const { mutate, isLoading, failureReason, isError } = useMutationNews({
        fn,
        currentPage,
        SuccessMsg: msgs.SuccessMsg,
    ...
    const onSubmit = (data: FormData) => {
        mutate({
            _newsId: news?._newsId,
            _title: data.Titulo.trim(),
            _description: data.Descripcion.trim(),
            _startDate: data.FechaInicio.trim(),
            _endDate: data.FechaFin.trim(),
        });
        ...
        <>
            <AnimatePresence>
                {failureReason?.message ? (
                    <motion.div
                        className="w-full rounded-md min-h-8 flex items-center pl-3 border border-red-600 bg-rose-200"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
            ...
            >
                <div className="flex flex-col gap-2">
                    <label className="sm:text-lg font-semibold text-gray-800">
                        Título
                    </label>
                    <input
                        {...register("Titulo", {
                            ...
                            minLength: {
                                value: 6,
                                message:
                                    "El titulo debe tener 6 caracteres como minimo",
                            },
                        })}
                    ...
                    <textarea
                        {...register("Descripcion", {
                            ...
                            minLength: {
                                value: 12,
                                message:
                                    "La descripcion debe tener 12 caracteres como minimo",
                            },
                        })}
                    ...
                    <input
                        ...
                        {...register("FechaFin", {
                            ...
                            validate: (value) => {
                                ...
                                if (fin < hoy)
                                    return "Debe ser mayor o igual a hoy";
