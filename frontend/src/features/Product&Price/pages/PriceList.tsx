...
const LazyTablePrice = lazy(() =>
    import("../components/TablePrice").then((module) => ({
...
export default function PriceList() {
    ...
    // Scroll hacia arriba cuando se carga la página o cambia el producto
    useEffect(() => {
        window.scrollTo(0, 0);
    }, [idProducto]);
        ...
        return (
            <Box sx={{ width: "80%", mx: "auto", mt: 5 }}>
                ...
                >
                    <Typography
                        variant="h4"
                        sx={{ color: "#561d03", fontWeight: "bold" }}
                    >
                        Gestión de Precios
                    </Typography>
                    {/* Agregar button volver en caso de ser necesario */}
                </Box>
                <Alert severity="error">Error al cargar precio</Alert>
            </Box>
        );
    }
    ...
    return (
        <div className="flex flex-col w-[80%] mx-auto my-4">
            ...
            {priceList.length === 0 ? (
                <Alert severity="info">
                    No existen precios registrados para el producto:{" "}
                    {idProducto}.
                </Alert>
            ) : (
                <>
                    <p className="text-gray-800 mb-2">
                        Producto: {priceList[0].producto.idProducto} -{" "}
                        {priceList[0].producto.nombre}
                    </p>
                    {/* Tabla de precios */}
                    <Suspense
                        fallback={
