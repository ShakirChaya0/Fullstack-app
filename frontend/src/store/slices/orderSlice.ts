...
import type {
    ...
    OrderStatus,
    Pedido,
} from "../../features/Order/interfaces/Order";
import type {
    Bebida,
    Comida,
} from "../../features/Products/interfaces/products";
import type { OrderLineStatus } from "../../features/KitchenOrders/types/SharedTypes";

const defaultState: Pedido = {
    idPedido: 0,
    lineasPedido: [],
    estado: "Solicitado",
    observaciones: "",
...
const initialState: Pedido = (() => {
    ...
    if (persistedState) return JSON.parse(persistedState) as Pedido;
    return defaultState;
})();
...
export const orderSlice = createSlice({
    ...
    reducers: {
        addToCart: (state, action: PayloadAction<Comida | Bebida>) => {
            // Obtener el máximo lineNumber de todos los arrays lineNumbers
            const allLineNumbers = state.lineasPedido.flatMap(
                (l) => l.lineNumbers ?? [],
            );
            const maxLineNumber =
                allLineNumbers.length > 0 ? Math.max(...allLineNumbers) : 0;
            ...
            const pendingIndex = state.lineasPedido
                ...
                .filter(
                    ({ lp }) =>
                        lp.producto._name === action.payload._name &&
                        lp.estado === "Pendiente",
                )
                .pop()?.i;
            ...
            if (pendingIndex !== undefined) {
                ...
                state.lineasPedido.push({
                    ...
                    lineNumbers: [maxLineNumber + 1],
                    estado: "Pendiente",
                    cantidad: 1,
                    subtotal: action.payload._price,
                    tipo:
                        "_type" in action.payload
                            ? (action.payload as Comida)._type
                            : undefined,
                    esAlcoholica:
                        "_isAlcoholic" in action.payload
                            ? (action.payload as Bebida)._isAlcoholic
                            : undefined,
                });
            }
        },
        removeFromCart: (state, action: PayloadAction<string>) => {
            ...
            const index = state.lineasPedido
                ...
                .filter(
                    ({ lp }) =>
                        lp.producto._name === action.payload &&
                        lp.estado === "Pendiente",
                )
                .pop()?.i;
        ...
        assignLineNumbers: (
            ...
            const index = state.lineasPedido.findIndex(
                (lp) => lp.producto._name === action.payload.nombreProducto,
            );
            if (index !== -1) {
                state.lineasPedido[index].lineNumbers =
                    action.payload.lineNumbers;
            }
        },
        modifyStatus: (
            state,
            action: PayloadAction<{
                ...
                orderLinesData: OrderLineClientInfo[];
            }>,
        ) => {
            ...
            state.lineasPedido.forEach((each) => {
                // Buscar por cualquier lineNumber en el array lineNumbers
                const matchingLine = orderLinesData.find((serverLine) =>
                    each.lineNumbers?.includes(serverLine.nroLinea ?? -1),
                );
